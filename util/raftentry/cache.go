...
// of indices.
func (c *Cache) Add(id rangeID, ents []raftpb.Entry, truncate bool) {
	if len(ents) == 0 {
		return
	}
...
func (c *Cache) Clear(id rangeID, hi uint64) {
	c.mu.Lock()
	p := c.getPartLocked(id, false /* create */, false /* recordUse */)
	if p == nil {
		c.mu.Unlock()
...
func (c *Cache) Get(id rangeID, idx uint64) (e raftpb.Entry, ok bool) {
	c.mu.Lock()
...
// entries.
func (c *Cache) Scan(
	ents []raftpb.Entry, id rangeID, lo, hi, maxBytes uint64,
) (_ []raftpb.Entry, bytes uint64, nextIdx uint64, exceededMaxBytes bool) {
	c.mu.Lock()
	p := c.getPartLocked(id, false /* create */, true /* recordUse */)
	c.mu.Unlock()
...
func (c *Cache) getPartLocked(id rangeID, create, recordUse bool) *partition {
	part := c.parts[id]
	if create && part == nil {
...
func newPartition(id rangeID) *partition {
	return &partition{
		id:   id,
		size: initialSize,
	}
